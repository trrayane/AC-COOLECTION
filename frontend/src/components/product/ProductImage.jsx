...
export function ProductImage({ p, color, view = 'front' }) {
  const photo = p.photos && p.photos.length ? p.photos[0].url : null;
  if (photo) {
    return <img src={photo} alt={p.name_en} loading="lazy" decoding="async" style={{ width: '100%', height: '100%', objectFit: 'cover', display: 'block' }} />;
  }
